...
CDCodec::CDCodec(int CDType, int n, int m, int r, CryptoPrimitive *cryptoObj)
{
    ...
    if ((CDType_ == AONT_RS_TYPE) || (CDType_ == OLD_CAONT_RS_TYPE) ||
        (CDType_ == CAONT_RS_TYPE) || (CDType_ == NONE_ENCRPT))
    { /*CDCodec based on AONT-RS, old CAONT-RS, or CAONT-RS*/
        // if (n <= 0)
        // {
        //     fprintf(stderr, "Error: n should be > 0!\n");
        //     exit(1);
...
bool CDCodec::squareMatrixInverting()
{
    ...
    for (i = 0; i < k_; i++)
    {
        ...
        for (j = i + 1; j < k_; j++)
        {
            ...
            if (squareMatrix_[h] != 0)
            { /*we need to do this when the i-th element in the j-th row is not equal to zero*/
                             ...
                             unsigned char *shareBuffer, int *shareSize)
{
    ...
    else
    {
        alignedSecretSize = (bytesPerSecretWord_ * k_) *
                                (((secretSize + bytesPerSecretWord_) / (bytesPerSecretWord_ * k_)) + 1) -
                            bytesPerSecretWord_;
    }
    if (alignedSecretBufferSize_ < alignedSecretSize)
    {
...
bool CDCodec::none_coding(unsigned char *secretBuffer, int secretSize,
                          unsigned char *shareBuffer, int *shareSize)
{
    for (int i = 0; i < secretSize; i++)
        printf("%d ", (int)secretBuffer[i]);
    printf("\n");
    ...
    memset(erasureCodingData_, 1, alignedSecretSize + bytesPerSecretWord_);
    memcpy(erasureCodingData_, alignedSecretBuffer_, alignedSecretSize);

    /*Step 2: generate the n shares from  the CAONT package using systematic Cauchy RS code*/

    /*directly copy the CAONT package from erasureCodingData_ to shareBuffer as the first k shares*/
    memcpy(shareBuffer, erasureCodingData_, alignedSecretSize + bytesPerSecretWord_);
    for (int i = 0; i < alignedSecretSize + bytesPerSecretWord_; i++)
        printf("%d ", (int)shareBuffer[i]);
    printf("\n");

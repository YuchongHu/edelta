...
template <class T>
struct span_data<T, typename std::enable_if<std::is_pointer<decltype(std::declval<T &>().data())>::value>::type> {
...
struct span_is_range {
    static constexpr bool value =
        ...
        !span_is_span<typename span_uncvref<R>::type>::value && !span_is_array<typename span_uncvref<R>::type>::value &&
        !std::is_array<typename span_uncvref<R>::type>::value && span_has_data<R, T>::value && span_has_size<R>::value;
};
...
struct span_copyable {
    static constexpr bool value =
        (N == boost::dynamic_extent || span_capacity<E, N>::value) && span_convertible<U, T>::value;
};
...
public:
    ...
    template <class U, std::size_t N,
              typename std::enable_if<detail::span_compatible<T, E, U, N>::value, int>::type = 0>
    ...
    constexpr span(R &&r) noexcept(noexcept(r.data()) &&noexcept(r.size())) : s_(r.data(), r.size()) {
    }

    template <class R,
              typename std::enable_if<E != dynamic_extent && detail::span_is_range<R, T>::value, int>::type = 0>
    explicit constexpr span(R &&r) noexcept(noexcept(r.data()) &&noexcept(r.size())) : s_(r.data(), r.size()) {
    }
    ...
    constexpr typename std::enable_if<C == dynamic_extent, span<T, detail::span_sub<E, O>::value>>::type
    subspan() const {

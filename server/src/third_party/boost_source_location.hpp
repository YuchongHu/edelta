...
public:
    ...
    BOOST_CONSTEXPR source_location(char const *file, boost::uint_least32_t ln, char const *function,
                                    boost::uint_least32_t col = 0) BOOST_NOEXCEPT : file_(file),
                                                                                    function_(function),
                                                                                    line_(ln),
                                                                                    column_(col) {
    }
    ...
    BOOST_CONSTEXPR source_location(std::source_location const &loc) BOOST_NOEXCEPT : file_(loc.file_name()),
...
#pragma warning(pop)
    ...
    inline friend bool operator==(source_location const &s1, source_location const &s2) BOOST_NOEXCEPT {
        return std::strcmp(s1.file_, s2.file_) == 0 && std::strcmp(s1.function_, s2.function_) == 0 &&
               s1.line_ == s2.line_ && s1.column_ == s2.column_;
    }
...
#define BOOST_CURRENT_LOCATION \
    ::boost::source_location(__builtin_FILE(), __builtin_LINE(), __builtin_FUNCTION(), __builtin_COLUMN())

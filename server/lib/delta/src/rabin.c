...
void finesse_super_feature(const unsigned char* p, int n, struct superF* SF) {
    UINT64 Feature[8], FeatureGroup[8];
    UINT64 fingerprint = 0;
    int number = 3;
    int i = 48, j = 0, bufPos = -1, jj = 0, m = 0;
    int N = number * NUMFEATURE;
    int sublength = n / N;
    int offset = 48;
    int start, end;
    ...
    unsigned char buf[128];
    memset((char*) Feature, 0, sizeof(Feature));
    memset((char*) FeatureGroup, 0, sizeof(FeatureGroup));

    for(m = 0; m < N; m++) {
        start = offset + m * sublength;
        end = start + sublength;
        fingerprint = 0;
        bufpos = -1;
        memset((char*)buf, 0, 128);
        for(j = 48; j >= 2; j--)
            SLIDE(p[start - j], fingerprint, bufPos, buf);
        for(i = start; i <= end; i++) {
            SLIDE(p[i - 1], fingerprint, bufPos, buf);
            if(fingerprint > Feature[m]) Feature[m] = fingerprint;
        }
    }

    Feature[0] > Feature[1] ? SWAP(Feature[0], Feature[1]) : 1;
    Feature[0] > Feature[2] ? SWAP(Feature[0], Feature[2]) : 1;
    Feature[1] > Feature[2] ? SWAP(Feature[1], Feature[2]) : 1;

    Feature[3] > Feature[4] ? SWAP(Feature[3], Feature[4]) : 1;
    Feature[3] > Feature[5] ? SWAP(Feature[3], Feature[5]) : 1;
    Feature[4] > Feature[5] ? SWAP(Feature[4], Feature[5]) : 1;
    
    FeatureGroup[0] = Feature[2];
    FeatureGroup[1] = Feature[5];
    FeatureGroup[2] = Feature[1];
    FeatureGroup[3] = Feature[4];
    FeatureGroup[4] = Feature[0];
    FeatureGroup[5] = Feature[3];


    jj = 0;
    Tempbuf = (unsigned char*) (FeatureGroup + NUMFEATURE * jj);
    uint64_t temp_hash = spooky_hash64(Tempbuf, 16, 12345678);
    SF->sf1 = temp_hash;
